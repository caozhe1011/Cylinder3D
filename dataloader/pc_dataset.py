...
def register_dataset(cls, name=None):  # 将类名：类的对象存入字典
...
@register_dataset  # 表示先调用register_dataset函数，在import时候就已经导入
class SemKITTI_sk(data.Dataset):
    ...
    def __getitem__(self, index):
        raw_data = np.fromfile(self.im_idx[index], dtype=np.float32).reshape((-1, 4))  # 一张点云中的数据
        if self.imageset == 'test':
            annotated_data = np.expand_dims(np.zeros_like(raw_data[:, 0], dtype=int), axis=1)
        else:
            annotated_data = np.fromfile(self.im_idx[index].replace('velodyne', 'labels')[:-3] + 'label',
                                         dtype=np.int32).reshape((-1, 1))  # 打开label文件
            annotated_data = annotated_data & 0xFFFF  # delete high 16 digits binary，低16位是标签
            annotated_data = np.vectorize(self.learning_map.__getitem__)(annotated_data)

        data_tuple = (raw_data[:, :3], annotated_data.astype(np.uint8))  # 将data与label拼起来
        if self.return_ref:  # 是否返回反射率
            data_tuple += (raw_data[:, 3],)  # [x,y,z,label,反射率]
        return data_tuple


@register_dataset  # 表示先调用register_dataset函数，在import时候就已经导入
class Rail_sk(data.Dataset):
    def __init__(self, data_path, imageset='train',
                 return_ref=False, label_mapping="rail-mapping.yaml", val_folder='a3', nusc=None):
        self.return_ref = return_ref
        with open(label_mapping, 'r') as stream:
            rail_yaml = yaml.safe_load(stream)
        self.learning_map = rail_yaml['learning_map']
        self.imageset = imageset
        if imageset == 'train':
            split = rail_yaml['split']['train']
        elif imageset == 'val':
            split = rail_yaml['split']['valid'][val_folder]
        else:
            raise Exception('Split must be train/val/test')

        self.im_idx = []  # 获取点云数据所在的文件名
        for i_folder in split:
            self.im_idx += absoluteFilePaths('/'.join([data_path, str(i_folder)]))
    ...
    def __getitem__(self, index):
        # raw_data = np.fromfile(self.im_idx[index], dtype=np.float32).reshape((-1, 4))  # 一张点云中的数据
        raw_data = np.load(self.im_idx[index])  # 一张点云中的数据
        raw_data[:, [0, 2]] = raw_data[:, [2, 0]]  # 交换为和kitti一样的坐标轴顺序 z,y,x
        if self.imageset == 'test':
            annotated_data = np.expand_dims(np.zeros_like(raw_data[:, 0], dtype=int), axis=1)
        else:
            annotated_data = raw_data[:, -1]
            annotated_data = annotated_data[:, None]
        data_tuple = (raw_data[:, :3], annotated_data.astype(np.int))  # 将data与label拼起来
        if self.return_ref:  # 是否返回反射率
            data_tuple += (raw_data[:, 3],)  # [z,y,x,label,反射率]
        return data_tuple
...
def absoluteFilePaths(directory):
    for dirpath, _, filenames in os.walk(directory):  # 获取目录路径，文件名
        filenames = sorted(filenames, key=lambda x: int(x[:-4]))
        for f in filenames:
            yield os.path.abspath(os.path.join(dirpath, f))  # 返回文件的生成器
...
def get_SemKITTI_label_name(label_mapping):
    with open(label_mapping, 'r') as stream:
        semkittiyaml = yaml.safe_load(stream)  # 读取yml
    SemKITTI_label_name = dict()
    for i in sorted(list(semkittiyaml['learning_map'].keys()))[::-1]:  # 反向读取，保留最后的mapping类型（mapping中有多类归结到一类）
