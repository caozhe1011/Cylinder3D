...
def build(model_config):
    ...
    cy_fea_net = cylinder_fea(grid_size=output_shape,  # 网络中下半部分的MLP
                              fea_dim=fea_dim,
                              out_pt_fea_dim=out_fea_dim,
                              fea_compre=num_input_features)
    # 初始化cylinder_asym类,同时调用cylinder_fea，Asymm_3d_spconv
    model = get_model_class(model_config["model_architecture"])(  # model_architecture: "cylinder_asym"
