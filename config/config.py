...
def load_config_data(path: str) -> dict:
    ...
    if schema_version not in SCHEMA_FORMAT_VERSION_TO_SCHEMA:
        raise Exception(f"Unsupported schema format version: {schema_version}.")  # 字符串前加f，在{}中表示python表达式

    strict_cfg = load(yaml_string, schema=SCHEMA_FORMAT_VERSION_TO_SCHEMA[schema_version])  # 按照封装好的键读取为YAML封装
    cfg: dict = strict_cfg.data  # 读取键值对，返回字典格式
    return cfg
